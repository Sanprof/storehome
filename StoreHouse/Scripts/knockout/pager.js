(function (window) {
    ...
    var pagerJsModule = function ($, ko) {
        ...
        p.path = function (path) {
            ...
            if (p && typeof (p) === 'object' && p.params && !(p instanceof pager.Page)) {
                if (p.path) {
                    ...
                    for (var i = 0; i < fullRoute.length; i++) {
                        var ind = fullRoute[i].indexOf('?');
                        if (ind >= 0) {
                            fRoute.push(fullRoute[i].substring(0, ind));
                        } else {
                            fRoute.push(fullRoute[i]);
                        }
                    }
                    for (var i = 0; i < fullRoute.length; i++) {
                        objectPath += '../';
                    }
                    objectPath += fRoute.join('/');
                }
                var params = p.params;
